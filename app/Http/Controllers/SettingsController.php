...

	{
		...
		try {
...

			{
				...
				$update_array = array('value' => $request->site_footer1_title);
			...
			if($request->hasfile('site_footer_map'))

			{

				$site_footer_map = config('site.footer_map');

				if($site_footer_map!='' && file_exists(public_path().'/uploads/'.$site_footer_map))

				{

					unlink(public_path().'/uploads/'.$site_footer_map);

				}

				$site_footer_map = $request->file('site_footer_map');

				$filename1 = $site_footer_map->getClientOriginalName();
				...
				$site_footer_map->move(public_path().'/uploads/', $filename1);  
				...
				$update_array = array('value' => $request->site_footer2_title);

				Settings::where('id', '24')

				->update($update_array);



				$update_array = array('value' => $request->site_footer2_sub_title);
				...
				$update_array = array('value' => $request->site_footer3_title);
				...
				$update_array = array('value' => $request->site_footer4_title);
				...
				$update_array = array('value' => $request->site_footer5_title);

				Settings::where('id', '28')

				->update($update_array);



				$update_array = array('value' => $request->site_footer5_sub_title);
